...
const Render = (input, SETTINGS = {}, debug = DEBUG.enabled, debuglevel = DEBUG.level, size = Size) => {
  ...
  if (OPTIONS.env === 'browser') {
    ...
    write = `<div style="font-family:monospace;white-space:pre;text-align:${OPTIONS.align};max-width:20%;overflow:scroll;background:${bgColor ? bgColor : 'transparent'}">` + `${write}` + `</div>`;
  }
